...
void HTNavGazeboRosAckermannDrivePrivate::OnUpdate(const gazebo::common::UpdateInfo & _info)
{
  ...
  auto linear_vel_left  = joints_[REAR_LEFT]->GetVelocity(0);
  auto linear_vel_right = joints_[REAR_RIGHT]->GetVelocity(0);
  auto target_linear = ignition::math::clamp(target_linear_, -max_speed_, max_speed_);
  double linear_diff_left = linear_vel_left - target_linear / wheel_radius_;
  double linear_cmd_left = pid_linear_vel_.Update(linear_diff_left, seconds_since_last_update);
  double linear_diff_right = linear_vel_right - target_linear / wheel_radius_;
  double linear_cmd_right = pid_linear_vel_.Update(linear_diff_right, seconds_since_last_update);
  ...
  if ( abs(past_target_rot_ - target_rot_) < 1e-5 && counter_ > 25){
    joints_[STEER_LEFT]->SetPosition(0, target_left_steering);
    joints_[STEER_RIGHT]->SetPosition(0, target_right_steering);  
    counter_ +=1;    
  }
  else if (abs(past_target_rot_ - target_rot_) > 1e-5 && counter_ > 25 ){
      counter_ = 0;
  }
  else{
